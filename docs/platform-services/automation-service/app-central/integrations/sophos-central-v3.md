---
title: Sophos Central V3
description: ''
tags: []
---

![](/img/platform-services/automation-service/app-central/logos/sophos-central-v3.png)

Version: 3.2  
Updated: Jul 11, 2023

Utilize Sophos Central enrichment data during incident investigations.

## Actions

* **Get Alerts** (*Enrichment)* - Gather Sophos Central alerts
* **Get Alerts Sophos Daemon** (*Daemon)* - Get Sophos Central alerts on a time interval
* **Get Endpoints** (*Enrichment)* - Gather all endpoints
* **Isolate an Endpoint** *(Containment) -* Isolate a single endpoint
* **Isolate Endpoints** (*Containment*) - Isolate multiple endpoints

## Sophos Central V3 configuration

The following steps show how to create new API credentials in order to work with Cloud SOAR.

1. Go to [https://central.sophos.com/manage/partner](https://central.sophos.com/manage/partner) and log into Sophos Central Partner.

1. On the left click on Settings & Policies and then click the API credentials link. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-1.png)

1. Click on Add Credential button. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-2.png)

1. Enter Credential name (required) and the description if you want. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-3.png)

1. Click the Copy button on the Client ID and paste it temporally in a text editor.

1. Next click Show Client Secret. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-4.png)

1. Now you can copy the key as shown. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-5.png)

## Sophos Central V3 in Automation Service and Cloud SOAR

1. To configure the integration, log into the application, expand the configuration menu in the top right corner by hovering over the gear icon and click Automation. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-6.png)

1. In the Automation section, on the left menu, click Integrations. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-7.png)

1. After the list of the integrations appears, search/look for the integration and click on the row.

1. The integration details will appear. Click on the "+" button to add new Resource. <br/>![](data:image/png;base64,)

1. Populate all the required fields (\*) and then click Save.
   * Label: the name of the resource.
   * URL: `https://api.central.sophos.com`
   * Client ID and Client Secret taken earlier from Sophos. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-9.png)

1. To make sure the resource is working, hover over the resource and then click the pencil icon that appears on the right. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-10.png)

1. Click Test Saved Settings. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-11.png)

1. You should receive a successful notification in the bottom right corner. <br/>![](/img/platform-services/automation-service/app-central/integrations/sophos-central-v3/sophos-central-v3-12.png)

## Change Log

* December 28, 2021 - First upload
* January 24, 2022 - New actions added
* July 11, 2023 (v3.2)
	+ Updated the integration with Environmental Variables
	+ Integration renamed from Sophos Central 3.0 to Sophos Central V3
