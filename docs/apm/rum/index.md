...
* Measure the following performance timings
  ...
  | Name | Calculation |
  |:---|:---|
  | `browser_time_to_first_xhr` | time from UI interaction till first HTTP `POST` appears |
  | `browser_time_to_last_xhr` | time from UI interaction till last HTTP `POST` ends) |
  | `browser_time_to_xhr_processing_end` | time from UI interaction till all browser-side processing of all XHR requests is done |
