...
1. Navigate to the **Configuration > Environment variables** section and set up the following environment variables (the first are three **required**):
   ...
   * `SUMO_OTLP_HTTP_ENDPOINT_URL` has to be set to send all gathered telemetry data to Sumo Logic. The URL comes from an [OTLP/HTTP source](/docs/send-data/hosted-collectors/http-source/otlp). You can use an existing Source or create a new one, if needed.
    ...
    :::
   * `SUMO_OTEL_SUPPRESS_AWS_INSTRUMENTATION` (optional, default: true, values: true/false). When true, suppresses instrumentation of internal AWS SDK calls made by the Lambda runtime. Set to false if you need spans for downstream AWS service calls (DynamoDB, S3, SQS, etc.).
...
[Sumo Logic Distribution for OpenTelemetry Lambda Layer version 2.0.2](https://github.com/SumoLogic/sumologic-otel-lambda/tree/release-nodejs-v2.0.2/nodejs) also provides packed [OpenTelemetry NodeJS](https://github.com/open-telemetry/opentelemetry-js) libraries for container-based Lambda functions.
...
1. Download and extract Sumo Logic OTel Lambda archive with instrumentation packages specific for your architecture - [amd64 (x86_64)](https://github.com/SumoLogic/sumologic-otel-lambda/releases/download/nodejs-v2.0.2/opentelemetry-nodejs-amd64.zip) or [arm64](https://github.com/SumoLogic/sumologic-otel-lambda/releases/download/nodejs-v2.0.2/opentelemetry-nodejs-arm64.zip).
