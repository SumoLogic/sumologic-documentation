...
Search templates work with [lookup (classic)](docs/search/search-query-language/operators#lookup-classic). They are not supported with our newer [lookup tables](/docs/search/lookup-tables).
...
1. Optional. Select the available values.
    ...
    1. For Label-Value pairs, copy paste the label-value pairs as comma-delimited lines. If you're using a Lookup make sure that you are using a valid [lookup (classic)](docs/search/search-query-language/operators#lookup-classic) file because the system will reject any lookup file path that it can't validate.  
    ...
    1. For a lookup file, you must enter a valid [lookup (classic)](docs/search/search-query-language/operators#lookup-classic) file that you have [saved](docs/search/search-query-language/operators#save-lookups-classic). 
...
1. Enter in a valid [lookup (classic)](docs/search/search-query-language/operators#lookup-classic) file that you have [saved](docs/search/search-query-language/operators#save-lookups-classic) with the save operator.  
