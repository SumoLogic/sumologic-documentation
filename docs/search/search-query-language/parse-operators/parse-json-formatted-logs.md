...
In addition, you can assign names to fields that differ from their original key names. To use `aID` instead of `accountId` and `eName` instead of `eventName`, you could use the `as` option like this:
...
![json results](/img/snippet/query-search/json_results_baselineIntervals.png)
...
Use the [nodrop](Parse-nodrop-option.md "Parse nodrop option") option to prevent this optimization. For example, the following query is looking for the key `event` and it has specified not to drop messages that do not have this key:
