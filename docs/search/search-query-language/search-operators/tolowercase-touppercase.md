...
description: Use the `toLowerCase` and `toUpperCase` operators to convert strings to all lowercase or uppercase letters. Normalize inconsistently capitalized logs like Windows Event logs, standardize filenames and paths, or reduce groups in conditional queries. Essential for case-insensitive matching with case-sensitive operators like equal to and not equal to signs.
---
