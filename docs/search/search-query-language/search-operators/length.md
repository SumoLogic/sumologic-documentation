...
description: Use the `length` operator to return the number of characters in a string field. Works only on string fields and returns 0 for null strings. Useful in where clauses to filter by string length, find fields with substantial text content, or identify abnormally long strings. Common for detecting payloads exceeding thresholds or validating field content length.
---
