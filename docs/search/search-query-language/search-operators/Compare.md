...
The compare operator can be used with the **Time Compare** button in the Sumo interface, which automatically generates the appropriate syntax and adds it to your aggregate query. See [Time Compare](../../time-compare.md) for details. The following information can also be found documented in Time Compare.
