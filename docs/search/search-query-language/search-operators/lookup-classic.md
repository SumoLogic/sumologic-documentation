...
description: Use the `lookup` (classic) operator to enrich log messages with data from the classic Lookup Tables feature. Maps values like user IDs to real names or finds deny-listed IP addresses using tables generated by the save operator or CSV files hosted on HTTPS servers. This is the legacy version—for newer deployments, use the current lookup operator with the scalable Lookup Tables feature.
---
