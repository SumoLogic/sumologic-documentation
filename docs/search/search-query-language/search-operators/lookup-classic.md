...
* If looking up a [metadata](../../get-started-with-search/search-basics/built-in-metadata.md) field you need to either [rename the field](as-operator.md) using the `as` option in the lookup operation or [filter null values](isNull.md) immediately after the lookup operation.
