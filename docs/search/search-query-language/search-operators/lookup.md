...
The `lookup` operator can return one or more fields from a lookup table hosted by Sumo Logic and add the fields to the log messages returned by your query. You create a lookup table using the lookup UI or the [Lookup API](https://api.sumologic.com/docs/#tag/lookupManagement). You can populate a lookup table by uploading a CSV file using the Lookup API, or by using the [save operator](save.md) to save the results of a log query. 

For information about lookup tables, see [Create and Manage Lookup Tables](../../lookup-tables/create-a-lookup-table.md).
...
To find a mismatch from a `lookup`  query, use the [isNull](isNull.md) operator.
