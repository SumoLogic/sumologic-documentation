...
description: Use the ASN Lookup operator to retrieve Autonomous System Number (ASN) and organization information for any IP address in your log data. Returns the ASN, the organization responsible for routing traffic, and the registering organization associated with the IP block. Valuable for network security analysis, identifying traffic sources by carrier or ISP, and investigating suspicious IP addresses. IP addresses without an ASN return null values.
---
