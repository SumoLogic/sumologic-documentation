...
* The entire [_raw](../../get-started-with-search/search-basics/built-in-metadata.md) message is counted when no field to count is provided.
...
When you want to count more than one field, you must create an alias using the [as operator](../search-operators/as-operator.md) to rename the `_count` fields.
...
By default, ordering is not defined inside of groups created using a group-by expression. To order your results, use the [**sort**](../search-operators/sort.md) operator.
...
Also, note that when you want to count the distinct occurrences of more than one field, you must create an alias using the [as operator](../search-operators/as-operator.md) to rename the \_count_distinct fields. See this example:
