...
1. Enter `./livetail -h`
1. When you run livetail the first time you need to provide the [deployment](/docs/api/about-apis/getting-started/#sumo-logic-endpoints-by-deployment-and-firewall-security) of the account and [access ID/key](/docs/manage/security/access-keys.md) to authenticate.<br/>  ![session start](/img/search/livetail/cli-live-tail-sessions-start.png)
