...
Follow the below steps to perform child org level search:
...
1. Enter the required query and hit the search button to obtain the search results.

:::note
- We only support selecting *one* child org to perform the log search.
- All the search that run via the child org in the parent org would be billed under child org account.
