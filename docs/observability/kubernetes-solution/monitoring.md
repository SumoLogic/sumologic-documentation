...
In order to start monitoring your Kubernetes environment, you first have to setup data collection and install the core Kubernetes dashboards by following the steps in our [quickstart](docs/observability/kubernetes-solution/quickstart.md) guide.
...
1. Select the graph data point you are interested in. In the following screenshot, we selected a cell in a honeycomb chart. A panel appears on the right of the window with details and a list of related content links.<br/>  ![details pane in Explore.png](/img/kubernetes/details-pane-Explore.png)
...
1. To examine the details of the smaller set of signatures that appear, under Select Count we selected. Oftentimes when troubleshooting a problem, our lesser quantity contains the root cause.
