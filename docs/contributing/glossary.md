...
**[Admin recommended](/docs/manage/content-sharing/admin-mode/#move-important-content-to-admin-recommended)**. An area at the top of the Sumo Library in the Left-Nav that lists highlighted content. A Sumo user with the Manage Content capability can add content to the admin recommended list.
...
**[Content sharing](/docs/manage/content-sharing)**. A Sumo feature that allows you to share searches, dashboards, and folders with a user, a role, or combinations of the two.
...
**[Ephemeral](/docs/send-data/installed-collectors/collector-installation-reference/set-collector-as-ephemeral)**. Refers to data sources, such as containers, that are created and destroyed as appropriate, resulting in a high volume log data or metrics. In Sumo Logic, you can set a collector to be ephemeral—Sumo deletes an ephemeral collector automatically after it has been offline for 12 hours.
...
**[Log overlay](/docs/metrics/metric-charts/log-overlay-analyze-metrics-visualizations)**. A Sumo feature you can use to run a log query on the metric query page and visualize the count of matching log messages on the metric chart.
