...
NOTE: This section doesn't apply to all sources; only where needed. When it is needed, user must configure on source's website before configuring source in Sumo. For example, they may need to collect API key from customer platform for authorization and use it when configuring source in .
