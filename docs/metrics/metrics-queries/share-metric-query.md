...
There are two ways you can share a metric query with other Sumo Logic users:
...
You can share a saved metric query from the metric query tab or the Sumo Logic library. 
...
1. **Users and Roles**. Enter the user names and/or roles to receive access. For example, if you want to share the query with all users that have the Analyst role, enter Analyst.
