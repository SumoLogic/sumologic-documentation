...
For a single metrics query row, Sumo Logic limits the number of input time series to 1000 for non-aggregate queries, and 37000 for aggregate queries (queries that have an aggregate operator like avg and max). In addition, the total number of datapoints scanned by a single row of query limited at 500,000,000 (500M) raw datapoints, or 50M [rollup datapoints](https://help.sumologic.com/docs/metrics/introduction/metric-quantization/#rollup-types ).
