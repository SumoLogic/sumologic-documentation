...
sidebar_label: parse
description: Use the `parse` metrics operator to parse specified fields and create new fields for use in metrics queries. Each wildcard in the pattern corresponds to a new field. Supports both lazy (shortest match with *) and greedy (longest match with **) wildcard matching. For Graphite metrics, parses metric names by default.
---
