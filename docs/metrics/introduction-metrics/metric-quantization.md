...
By default, Sumo uses the `avg` rollup type. You can specify another rollup type by using the `quantize` operator, as described in [Quantize with rollup type specified](https://help.sumologic.com/Metrics/Introduction-to-Metrics/Metric_Quantization#Quantize_with_rollup_type_specified) below.
