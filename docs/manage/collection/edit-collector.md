...
1. Make your changes. Note that updated metadata is only be applied to newly ingested data; previously uploaded data retains its original metadata.
    ...
    * **Assign to a Budget** allows you to assign an [ingest budget] (../Ingestion-and-Volume/Ingest_Budgets.md) to the Collector. The drop-down displays your ingest budgets in the following format:
...
1. Click the Hosted Collector name, or click **Edit** to the right of the Collector name.
    ...
    ![edit hosted collector.png](/img/collector/edit-hosted-collector.png)
