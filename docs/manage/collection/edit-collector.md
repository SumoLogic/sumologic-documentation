...
1. Make your changes. Note that updated metadata is only be applied to newly ingested data; previously uploaded data retains its original metadata.
    ...
    * Click the **Add Field** link in the **Fields** section if you want to assign metadata [fields](../Fields.md "Fields") to the Collector. Define the fields you want to associate, each field needs a key and value.
      * ![green check circle.png](./static/img/03Edit-a-Collector/../../Fields/green%20check%20circle.png) A green circle with a check mark is shown when the field exists in the Fields table schema. 
      * ![orange exclamation point.png](./static/img/03Edit-a-Collector/../../Fields/orange%20exclamation%20point.png) An orange triangle with an exclamation point is shown when the field doesn't exist in the Fields table schema. In this case, an option to automatically add the nonexistent fields to the Fields table schema is provided. If a field is sent to Sumo that does not exist in the Fields schema it is ignored, known as dropped.

    * **Assign to a Budget** allows you to assign an [ingest budget](../Ingestion-and-Volume/Ingest_Budgets.md "Ingest Budgets") to the Collector. The drop-down displays your ingest budgets in the following format:

        `<budget name> (<field value>) (<allocated capacity>)`

1. Click **Advanced** to set a [CPU Usage Target](06Set-the-Collector-CPU-Usage-Target.md "Set the Collector CPU Usage Target"), or to change the Collector management method you're using. You can switch to **Cloud Based Management** at any time by changing the setting here and saving, your JSON configuration will be ignored once changed to Cloud Based. To switch to **Local Configuration File** there are a few steps you'll need to do, see [Local Configuration File Management](../../03Send-Data/Sources/03Use-JSON-to-Configure-Sources/Local-Configuration-File-Management.md "Local Configuration File Management") for details.   

    ![Edit installed collector.png](./static/img/03Edit-a-Collector/Edit%20installed%20collector.png)
...
1. Click the Hosted Collector name, or click **Edit** to the right of the Collector name.
    ...
    ![edit hosted collector.png](./static/img/03Edit-a-Collector/edit%20hosted%20collector.png)
...
1. Change the name or change the metadata fields as needed. Note that updated metadata is only be applied to newly ingested data; previously uploaded data retains its original metadata. 
   ...
   * Define the [**Fields**](../Fields.md "Fields") you want to associate, each field needs a key and value.  
