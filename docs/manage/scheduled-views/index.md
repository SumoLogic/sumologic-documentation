...
* Account Admins and users whose role grants the "Manage Scheduled Views" [role capability] (Users-and-Roles/Manage-Roles/05-Role-Capabilities.md "Role Capabilities") can set up Scheduled Views, but anyone in an organization can run searches against them. Other users' data access to a Scheduled View is governed by the search filters associated with their roles; they will only be able to see data to which their roles allow them access. For more information, see [Construct a Search Filter for a Role] (Users-and-Roles/Manage-Roles/07-Construct-a-Search-Filter-for-a-Role.md "Construct a Search Filter for a Role").
