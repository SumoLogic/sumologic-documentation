---
id: integrate-google-iam-/img/security
---

# Integrate Sumo Logic with Google Apps (G Suite) IAM /img/security
...
For key information about SAML in Sumo, see the [Limitations](01-Set-Up-SAML-for-Single-Sign-On.md "Set Up SAML for Single Sign-On") section of the "Set Up SAML for Single Sign-On" page.
...
1. On the **Enable SSO for SAML Application** page, select **Setup my own Custom App** at the bottom of the page**.
    ...
    ![](/img/security/ga_saml_enable_sso.png)
...
1. The **Google IdP Information** page appears. Make note of the following URLs, as you will supply them when you configure SAML in Sumo: 
   ...
   * **SSO URL**. You'll enter this URL as the **Authn Request URL** when you perform the steps in [Configure Sumo Logic SAML](./Integrate-Sumo-Logic-with-Google-Apps-(G-Suite)-IAM-/img/security.md "Integrate Sumo Logic with Google Apps (G Suite) IAM /img/security") below. 
   * **Entity ID**. You'll enter this URL as the **Issuer** in Sumo Logic when you perform the steps in [Configure Sumo Logic SAML](./Integrate-Sumo-Logic-with-Google-Apps-(G-Suite)-IAM-/img/security.md "Integrate Sumo Logic with Google Apps (G Suite) IAM /img/security") below.

    ![](/img/security/ga_saml_sso.png)

1. Click **Download** for the Certificate.
1. Click **Next**.
1. In **Basic Information for your Custom App** page, enter the following:

    ![](/img/security/ga_saml_basic.png) 
...
1. **Debug Mode.** Not required. Activating this setting now is useful for troubleshooting later. Select this option if you'd like to view additional details if an error occurs when a user attempts to authenticate. For more information, see [View SAML Debug Information](View-SAML-Debug-Information.md "View SAML Debug Information").
...
1. **Roles Attribute.** When you click this option, the **Roles** **Attribute** field appears. Enter the SAML Attribute Name that is sent by the IdP as part of the assertion. For details, see [Set Up SAML for Single Sign-On](01-Set-Up-SAML-for-Single-Sign-On.md "Set Up SAML for Single Sign-On").
1. **On-Demand provisioning.** Select this option and specify the following attributes to have Sumo Logic automatically create accounts when a user first logs on. For more information, see [Set Up SAML for Single Sign-On](01-Set-Up-SAML-for-Single-Sign-On.md "Set Up SAML for Single Sign-On"). 
...
1. **Logout Page**. Select this option and enter a URL if you'd like to point all users to the URL after logging out of Sumo Logic. For more information, see [Set Up SAML for Single Sign-On.](01-Set-Up-SAML-for-Single-Sign-On.md "Set Up SAML for Single Sign-On")
...
1. Go back to the **Google Auth Configuration – /img/security Provider Details** dialog, and enter the following information:
    ...
    ![](/img/security/ga_saml_/img/security_provider.png) 
...
1. In the **Attribute Mapping** dialog, make the following selections:
    ...
    ![](/img/security/ga_saml_attribute_mapping.png) 
...
1. After a short delay, the new Sumo Logic SAML App will be displayed in your Google Apps login menu. In some cases, it may take up to 10 minutes for the new app to appear. 
    ...
    ![](/img/security/ga_saml_google_menu.png)
