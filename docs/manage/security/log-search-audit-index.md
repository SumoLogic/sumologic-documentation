...
| Compare | Search queries that used the logreduce or compare timeshift operators generated two audit logs. Instead of generating two audit logs for the parent and child queries, a single audit log will now capture the scanned bytes for both queries and store them in a single audit index. As a result, the scanned bytes for Compare queries will be found as a sum of the parent and child queries in the parent query audit index. |
| Subquery | Subqueries that were run as a separate search with corresponding attributes captured in the Search Audit Index generated two audit logs. Instead of generating two audit logs for the parent and child queries, a single audit log will now capture the scanned bytes for both queries and store them in a single audit index. As a result, the scanned bytes for Subquery queries will be found as a sum of the parent and child queries in the parent query audit index.   |
