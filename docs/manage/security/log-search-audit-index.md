...
| Compare | Search Queries that use logreduce or compare timeshift operators are no longer supported. Instead of generating two audit logs for the parent query and the child query, a single audit log will now capture the scanned bytes for both queries and store them in a single audit index. As a result, the scanned bytes for Compare queries will be found in the parent query audit index as a total of the parent and child queries. |
| Subquery | Subqueries that were run as a separate search with corresponding attributes captured in the Search Audit Index are no longer supported. Instead of generating two audit logs for the parent query and the child query, a single audit log will now capture the scanned bytes for both queries and store them in a single audit index. As a result, the scanned bytes for Subquery queries will be found in the parent query audit index as a total of the parent and child queries.     |
