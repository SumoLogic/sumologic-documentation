...
* Searches against SIEM data in Sumo Logic. Don't use `_dataTier` when searching SIEM data. Instead, use `_index` to specify the security partition or partitions you want to access, as described in [Searching for SIEM Records in Sumo Logic](/docs/cse/records-signals-entities-insights/search-cse-records-in-sumo.md).
