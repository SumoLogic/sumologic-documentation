...
1. Enter **-32d** for the time range.
    ...
    ![time range monthly plan.png](/img/ingestion-volume/daily-32d.png)
...
1. Substitute the correct value of `X` for the following parameter in the search query (see entry in yellow in the query below).
    ...
    ![Account](/img/ingestion-volume/account-overview.png)
...
1. Set time range value to **Last 24 Hours**.
    ...
    ![time range daily plan limt.png](/img/ingestion-volume/daily-last-24.png)
...
1. Enter **-65m -5m** for the time range.
    ...
    ![time range usage spike.png](/img/ingestion-volume/hourly-65.png)
...
If you don't want the results of the query across Sources or source categories to be called "collector", you can replace all 3 instances of "collector" with a different field name.
...
1. Set a time range. The default is **Last 24 hours**. If you need to allow for more time because some collectors do not typically ingest data that often, specify a longer time range. For example, seven days.
    ...
    ![Alert](/img/ingestion-volume/AlertDataLoss.png)
...
1. Set the time range to the **Last 15 Minutes**.
    ...
    ![time range throttling alert.png](/img/ingestion-volume/time-throttling.png)
