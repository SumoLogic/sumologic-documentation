...
This is an archive of 2018 Collector Release Notes. The current Collector Release Notes are [here](/docs/releasenotes/collector).
...
* Enhanced metadata allowing you to customize sourceCategory and sourceHost with environment variables, labels, and tags. Docker tags welcome, for more information see [Collect Logs and Stats from Docker](https://help.sumologic.com/07Sumo-Logic-Apps/10Containers_and_Orchestration/Docker_Apps/Docker/01-Collect-Logs-and-Metrics-from-Docker#Configure_sourceCategory_and_sourceHost_using_variables).
...
* When upgrading from version 19.209 to 19.216 on Ubuntu 16 and above using a shell executable file (.sh), Debian, or the Web UI you will need to [manually restart the collector service](https://help.sumologic.com/Manage/Collection/02Start-or-Stop-a-Collector-using-Scripts) after upgrading.
* Upgrading Collectors without administrator privileges on Windows to 19.216 or higher will require a [manual upgrade installation](https://help.sumologic.com/Manage/Collection/08Upgrade_Collectors_using_the_Command_Line).
