...
CyberArk Identity Security Platform is a robust identity management solution that heightens enterprise security with single sign-on, multi-factor authentication, and privileged access controls. It simplifies identity operations while delivering comprehensive protection against external and internal cyber threats.

The Audit service provides audit trails for activities, events, and sessions that are performed by any integrated service on the Shared Services platform. An audit trail is a record of activities that have occurred within a system. This trail can be used to track system changes and can be used for a variety of purposes, such as security, regulatory compliance, incident-response investigation, and troubleshooting.
...
1. **Identity ID**. Enter your identity ID from the [Vendor configuration](#vendor-configuration) section. For example, `abr4338`.
1. **Web Application ID**. Enter your application ID from the [Vendor configuration](#vendor-configuration) section. For example, `sumologic`.
1. **Username**. Enter your username(client-id) from the [Vendor configuration](#vendor-configuration) section. For example, `c2c@cyberark.cloud.30488`.
1. **Password**. Enter your password(client-secret) from the [Vendor configuration](#vendor-configuration) section.
1. **Tenant URL**. Enter your tenant URL from the [Vendor configuration](#vendor-configuration) section. For example, `https://sumologic.audit.cyberark.cloud`.
1. **API Key**. Enter your API key from the [Vendor configuration](#vendor-configuration) section.
...
1. **Polling Interval**. The polling interval is the frequency at which the CyberArk Audit C2C Source will check for updates from the CyberArk Audit API. This field is pre-filled with 5.
...
| pollingIntervalMin | integer | Yes | 5 | Frequency of C2C updates from CyberArk Audit. |  |  
