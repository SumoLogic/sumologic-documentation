...
The Slack integration ingests specific web and audit events.

- [Slack Web API](https://api.slack.com/web)
- [Slack Audit API](https://api.slack.com/admins/audit-logs)

## Data Sources
The following API endpoints and routes are collected by the integration:
...
## Meta-data Fields
The SIEM forward option is currently not supported. It will be added later once Cloud SIEM has supporting content. This
may also be complicated due to the C2C transforming some data for Sumo Logic app compatibility.

## Setup and Configuration Overview
Data can be collected from two Slack APIs known as the [Web API](https://api.slack.com/web) and the
[Audit API](https://api.slack.com/admins/audit-logs). The Web API is used to collect standard channel, user and message
information from a specific workspace. The Audit API is used to collect security audit events across the entire account
including all workspaces, but it requires a Slack Enterprise Grid license. Both APIs collect different information, and
you should collect both if you have a Slack Enterprise Grid license.

We recommend creating a Slack App for each Slack Workspace you want to monitor channel, user and message information
from. This will also require a Sumo Logic Slack C2C per Slack Workspace. If you have a Slack Enterprise Grid account,
you can create an additional Slack app, install it on the Enterprise Grid instead of a Workspace and create another Sumo
Logic C2C to monitor your Enterprise Grid audit logs from the audit api.

Summary Overview:

1. Create the Slack App with the correct permissions
2. Install the Slack app on a specific workspace to monitor Web API logs or install the app on the Enterprise Grid to
monitor Audit API logs.
...
3. Select `From scratch` if asked how you would like to configure your Slack app. <br/><img src={useBaseUrl('img/send-data/slack-create-app-from-scratch.png')} alt="From Scratch App]" width="<insert-pixel-number>"/>
...
instructions later for migrating it. <br/><img src={useBaseUrl('img/send-data/slack-create-app-name.png')} alt="App Name and Workspace Assignment]" width="<insert-pixel-number>"/>
5. Click `Create App`.
6. You are now presented with the basic information about your Slack app. Click on the `Permissions` link under the `Add features and functionality` section. <br/><img src={useBaseUrl('img/send-data/slack-basic-info-permissions.png')} alt="App Name and Workspace Assignment]" width="<insert-pixel-number>"/>
7. Scroll down to the `Scopes` section add multiple `User Token Scopes` depending on your Slack account type. <br/><img src={useBaseUrl('img/send-data/slack-scope-add.png')} alt="App Name and Workspace Assignment]" width="<insert-pixel-number>"/>

Use the table below to reference the required scope permissions you need to add depending on the Slack API you want to
collect along with your Slack account type:
...
1. On the app settings page, click `Install App` and the `Install to Workspace` button. <br/><img src={useBaseUrl('img/send-data/slack-install-app-to-workspace.png')} alt="slack-install-app-to-workspace.png" width="<insert-pixel-number>"/>
2. Allow your new Slack App to monitor your workspace. <br/><img src={useBaseUrl('img/send-data/slack-app-allow.png')} alt="Allow App to Monitor Workspace" width="<insert-pixel-number>"/>
3. Save the generated access token. This will be used in the Sumo collector configuration for access. <br/><img src={useBaseUrl('img/send-data/slack-copy-user-token.png')} alt="Copy Token" width="<insert-pixel-number>"/>

### Install the Slack App on the Enterprise Grid for Audit API Logs
Only follow these steps if you are installing a Slack App on the Enterprise to monitor Audit API logs. Please ensure you
have followed the prior steps for creating the Slack app with the appropriate permissions before continuing to this
section. A Slack Enterprise Grid account is required.

1. On the app settings page, click `OAuth & Permissions`. <br/><img src={useBaseUrl('img/send-data/slack-oath-perm-link.png')} alt="Oauth Permissions" width="<insert-pixel-number>"/>
2. Scroll down to the `Redirect URLs`. Add a new redirect URL as `https://localhost` and click `Save URLs`. <br/><img src={useBaseUrl('img/send-data/slack-redirect-url.png')} alt="Redirect URL" width="<insert-pixel-number>"/>
...
`I’ve reviewed and removed any hard-coded information`checkbox. Click the `Activate Public Distribution` button. <br/><img src={useBaseUrl('img/send-data/slack-activate-public-distro.png')} alt="Activate Public Distribution" width="<insert-pixel-number>"/>
...
12. From the response, save the token value from the field `access_token` as it will be used for the Sumo collector.
