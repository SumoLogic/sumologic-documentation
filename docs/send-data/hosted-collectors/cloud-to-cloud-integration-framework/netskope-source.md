...
1. **Event Types** (Optional). By default, all event types are collected. You can specify certain event types to collect. Make sure to have the corresponding token privileges to the event types. If this field is empty, all event types are collected. Be aware that if you want to collect all event types, if a new event type is added in the future, your token might need to be updated accordingly.
