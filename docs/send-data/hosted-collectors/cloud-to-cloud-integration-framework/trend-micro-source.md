...
When you create an Trend Micro source, you add it to a Hosted Collector. Before creating the Source, identify the Hosted Collector you want to use or create a new Hosted Collector. For instructions, see [Configure a Hosted Collector](/docs/send-data/hosted-collectors/configure-hosted-collector).

To configure an Trend Micro source:
...
Sources can be configured using UTF-8 encoded JSON files with the Collector Management API. See [SON to configure Sources](/docs/send-data/use-json-configure-sources) for details. 
