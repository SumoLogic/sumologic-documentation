...
| paginationType             | String      | Yes      | `"LinkHeaders"`   | Pagination type `LinkHeaders`, `ContinuationToken`, `Offset`, `PageBased`, or `None`. Each type has required companion fields (see the rows below); if they are missing, validation fails with misleading errors such as `<type> is not a valid enum value`.                                                                                                                                                                                     | `"LinkHeaders"`, `"None"`                                                                                                                                                                                                                                 |
