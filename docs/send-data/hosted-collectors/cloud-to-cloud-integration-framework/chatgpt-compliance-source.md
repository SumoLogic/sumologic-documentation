...
| :-- | :-- | :-- |
| Conversations | Log of user interactions with ChatGPT, including exchanged messages and metadata, used for compliance review and auditing. | 5 minutes to 24 hours |
| Codex | Logs of Codex interactions, capturing code generation and AI-assisted development activity for compliance review. | 5 minutes to 24 hours |
| Codex Security | Security-related events from Codex, including flagged or sensitive code generation requests. | 5 minutes to 24 hours |
| Audit | Audit trail events capturing administrative and user actions within the ChatGPT Enterprise workspace. | 5 minutes to 24 hours |
| App | Events related to installed applications and integrations within the ChatGPT Enterprise environment. | 5 minutes to 24 hours |
| Auth | Authentication and authorization events, including sign-in activity and access control changes. | 5 minutes to 24 hours |
| App Auth | Authorization events specific to application-level access and OAuth token activity. | 5 minutes to 24 hours |
...
1. **Polling Interval**. The polling interval is set to 1 hour by default and can be configured from a minimum of 5 minutes to a maximum of 24 hours. You can adjust it based on your needs. This sets how often the source checks for new data.
...
| enabledEventType | Array | Yes | `["CONVERSATION"]` | List of event types to collect. Supported values: `CONVERSATION`, `CODEX`, `CODEX_SECURITY`, `AUDIT`, `APP`, `AUTH`, `APP_AUTH`. | `["CONVERSATION", "AUDIT"]` |
| pollingIntervalHour | String | Yes | `1h`               | Time interval after which the source will check for new data. Minimum: `5m`, maximum: `24h`. | `5m`, `1h`, `24h` |
...
## Limitations
- Updates to events after a prolonged gap may cause re-ingestion and potential duplication.
- As ChatGPT continues to evolve, updates may alter event data or API behavior, potentially impacting integration consistency.
