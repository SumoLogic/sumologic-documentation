...
The Google Workspace integration consumes data from the Google Workspace [Users API](https://developers.google.com/admin-sdk/directory/reference/rest/v1/users/list). The Google Workspace Source retrieves data once per 24 hours based on user input for either deleted users or all users in a domain.
...
1. Provide the **Client Email**, **Private Key**, and **Token URL** you got in the JSON file after you created service account credentials in the [Creating Service Account](#creating-service-account) above.
