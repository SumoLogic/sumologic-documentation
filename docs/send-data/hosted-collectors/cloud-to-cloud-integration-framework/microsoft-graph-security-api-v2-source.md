---
id: microsoft-graph-security-api-v2-source
title: Microsoft Graph Security API V2 Source
sidebar_label: Microsoft Graph Security API V1
description: The Microsoft Graph Security API V2 Source provides a secure endpoint to receive alerts from the Microsoft Graph Security API endpoint.
---
...
The Microsoft Graph Security API V2 Source provides a secure endpoint to receive alerts from the [Microsoft Graph Security API](https://docs.microsoft.com/en-us/graph/overview) endpoint. It securely stores the required authentication, scheduling, and state tracking information. One threat event is reported for each affected device.

## Collected data

The Microsoft Graph Security API V2 Source consumes [alerts](https://learn.microsoft.com/en-us/graph/api/resources/security-alert?view=graph-rest-1.0) from the [Microsoft Graph Security API Endpoint](https://learn.microsoft.com/en-us/graph/api/security-list-alerts_v2?view=graph-rest-1.0).

## States

A Microsoft Graph Security API V2 Source tracks errors, reports its health, and start-up progress. You’re informed, in real-time, if the Source is having trouble connecting, if there's an error requiring user action, or if it is healthy and collecting by utilizing Health Events.

A Microsoft Graph Security API V2 Source goes through the following states when created:
...
When you create a Microsoft Graph Security API V2 Source, you add it to a Hosted Collector. Before creating the Source, identify the Hosted Collector you want to use or create a new Hosted Collector. For instructions, see [Configure a Hosted Collector](/docs/send-data/hosted-collectors/configure-hosted-collector).

To configure a Microsoft Graph Security API V2 Source:
...
The following table shows the **config** parameters for a Microsoft Graph Security API V2 Source.
...
Microsoft Graph Security API V2 Source JSON example:
