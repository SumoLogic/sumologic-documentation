---
id: claude-compliance-source-beta
...
| collectActivities | Boolean | No | `true` | Enable collection of compliance activity events (chat creation, file uploads, user sign-ins, admin actions). At least one of `collectChatMessages` or `collectActivities` must be `true`. | `true` |
