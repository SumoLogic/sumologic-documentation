...
<table>
  ...
  <tr>
   ...
   Permissions - User must have the <code>GAV/CSAM</code> module and the <code>App API Enabled</code> option enabled for that role. Additionally, the user must have the <code>Allow user view access to all objects</code> checkbox enabled under "Roles And Scopes" within the user settings<br />
