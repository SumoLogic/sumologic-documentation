...
Rate Limit is not applicable as we will use CloudQuery GCP SDK.
...
- For the SDK, if the response code is 401 and 403.
   - Config error and quit the integration
- For any other error returned by CloudQuery, we will set ErrorGeneric as an error in C2C.
   - Integration will not stop it’s execution
