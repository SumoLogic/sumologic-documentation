...
<img src={useBaseUrl('img/send-data/azure-event-hub.svg')} alt="Azure icon" width="40"/>
...
<>
  {(() => {
    ...
    useEffect(() => {
      fetch(useBaseUrl('/files/c2c/azure-metrics/example.json'))
        .then(res => res.text())
        .then(setJson);
    }, []);
    ...
    return (
      <>
        ...
        <a href={useBaseUrl('/files/c2c/azure-metrics/example.json')} target="_blank" rel="noopener noreferrer">Download example</a>
      </>
    );
  })()}
</>
