...
```
:::note
If you did a manual install of the collector, then the configuration might be in a different location.
:::

### 1. Sumo defined configuration 
This is required by collector to properly communicate with SumoLogic SaaS service, and tranmit data. All Sumo Logic preconfigured components are stored in `sumologic.yaml` file. This file is managed by the installation script and should never be changed manually. Depending upon your platform 
