...
For example JSON configurations, see [Use JSON to Configure Sources](Local-Configuration-File-Management/...md "Use JSON to Configure Sources"). The topic includes a table of [common parameters](Local-Configuration-File-Management/...md "Use JSON to Configure Sources") that apply to all Source types and links to the topics that describe the parameters for specific Source types.
...
`{   "api.version":"v1",   "source":{      "sourceType":"LocalFile",      "name":"Example1",      "pathExpression":"/var/logs/maillog"   } }`
...
`{   "api.version":"v1",   "sources":[      {         "sourceType":"LocalFile",         "name":"Example1",         "pathExpression":"/var/logs/maillog"      },{         "sourceType":"LocalFile",         "name":"Example2",         "pathExpression":"/var/logs/syslog"      }   ] }`

The maximum number of Sources allowed on a Collector is 1,000.

To set up new Collectors and Sources see [Local Configuration File
Management for New Collectors and
Sources](Local-Configuration-File-Management/Local-File-Configuration-Management-for-New-Collectors-and-Sources.md "Local Configuration File Management for New Collectors and Sources"). 

To convert existing Sources from Cloud Management to Local Configuration
File Management see [Local Configuration File Management for Existing
Collectors and
Sources](Local-Configuration-File-Management/Local-Configuration-File-Management-for-Existing-Collectors-and-Sources.md "Local Configuration File Management for Existing Collectors and Sources").

### Configure the location of JSON file or folder

When using local file configuration management you specify the location
of the JSON file or the folder that contains multiple JSON files in the
Collector’s `config/user.properties` file. You need to use the
`syncSources` parameter to point to your configuration file or folder.
