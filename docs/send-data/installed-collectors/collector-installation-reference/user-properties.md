...
| `category=category`<br/>Available on Collector version 19.182+. | Source category to use when a source does not specify a category. | No, use the [Collector Management API](/api/collectors) to modify. |
| `clobber=true/false` | When true, if there is any existing collector with the same name, that collector will be deleted (clobbered). See Forcing a Collector's Name with Clobber for more information. | No, use the [Collector Management API](/api/collectors) to modify. |
| `description=description`<br/>Available on Collector version 19.182+. | Description for the collector to appear in Sumo Logic. | No, use the [Collector Management API](/api/collectors) to modify. |
...
| `hostName=hostname`<br/>Available on Collector version 19.182+. | The host name of the machine on which the collector is running. The host name can be a maximum of 128 characters. | No, use the [Collector Management API](/api/collectors) to modify. |
