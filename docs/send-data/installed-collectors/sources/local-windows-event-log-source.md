...
1. Set the following:
   ...
   * **Windows Event Types.** Select the event types you want to collect:
     ...
     * **Forwarded Events**. See how to [Collect Forwarded Events from a Windows Event Collector.](collect-forwarded-events-windows-event-collector.md). Note this box is not seletable when the _Standard Event Channels_ is checked.
