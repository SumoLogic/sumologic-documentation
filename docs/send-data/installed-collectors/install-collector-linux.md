...
Follow the steps in this topic to install or uninstall a collector on Linux. See [Installed Collectors](04Install-a-Collector-on-Linux/...md "Installed Collectors") for information on other OSs.
...
* Open a browser and enter the static URL for your pod. See [Download a Collector from a Static URL](05Reference-Information-for-Collector-Installation/02Download-a-Collector-from-a-Static-URL.md "Download a Collector from a Static URL") for a list of URLs for your deployment pod. The download begins immediately.
...
You can build a Collector into a [Linux machine image](05Reference-Information-for-Collector-Installation/04Add-a-Collector-to-a-Linux-Machine-Image.md "Add a Collector to a Linux Machine Image")
such as an Amazon AMI or VMware image.
...
1. Configure the Collector `user.properties` file in the `/opt/SumoCollector/config/` directory. The Collector uses the settings defined in `user.properties` to register and start. See [user.properties](05Reference-Information-for-Collector-Installation/06user.properties.md "user.properties") for a full list of all the supported parameters.

   To use an [access key](/Manage/Security/Access-Keys), provide the `accessid` and `accesskey` parameters. For example:
   ...
   :::note
   Starting with collector 19.170+, the installation directory is secured to users belonging to the `sumologic_collector` group. Modifying `user.properties` may require sudo privileges. For more information see [Enhanced File System Security for Installed Collectors](05Reference-Information-for-Collector-Installation/08Enhanced-File-System-Security-for-Installed-Collectors.md "Enhanced File System Security for Installed Collectors").
   :::

1. (Optional) Provide a JSON Source information.  You can pass all Source settings in a UTF-8 encoded JSON file. If you are using a JSON file, you must provide the file before starting the Collector. See [Using JSON to configure Sources](../Sources/03Use-JSON-to-Configure-Sources.md "Use JSON to Configure Sources"). Alternatively, you can configure Sources at any time by using the Sumo web app. See [Sources](../Sources.md "Sources").
1. (Optional) Set the run a user for the Collector if you want the Collector to run as a user other than root. See [run as for a Collector](05Reference-Information-for-Collector-Installation/05Set-the-Run-As-User-for-a-Collector.md "Set the Run As User for a Collector").
...
1. (Optional) Set the run as user for the Collector if you want the Collector to run as a user other than root. See [run as for a Collector](05Reference-Information-for-Collector-Installation/05Set-the-Run-As-User-for-a-Collector.md "Set the Run As User for a Collector").
