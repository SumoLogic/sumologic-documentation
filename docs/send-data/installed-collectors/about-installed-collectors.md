...
See how to [design your deployment](/docs/send-data/design-deployment) for details on when to use a single or multiple Installed Collectors.
...
Generally, as the number of Sources increases, the number of threads
also increases. The Collector will use three threads per available CPU
by default, you can [increase the max
threads](../Collector-FAQs/Increase_Max_Threads_for_Collector.md "Increase Max Threads for Collector")
if needed.
