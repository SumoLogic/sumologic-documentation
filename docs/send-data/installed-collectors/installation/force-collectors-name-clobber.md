...
* Specify a unique name for the collector. You can do so during installation, or by editing the collector configuration following installation. On the [**Collection**](/docs/send-data/collection/) page, click **Edit** for the Collector.
...
To set the clobber flag via command line argument:
...
1. Verify that the generated [user.properties file](/docs/send-data/installed-collectors/collector-installation-reference/user-properties) (under the `config` directory) contains the property `clobber=true`.

## Using the Binary Package

To set the clobber flag via the [user.properties file](/docs/send-data/installed-collectors/collector-installation-reference/user-properties):

1. Create and edit the user.properties configuration file (in the `config` directory of your Collector installation).
