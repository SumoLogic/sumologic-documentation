...
sidebar_label: Install the Kubernetes Helm Chart
...
> [!NOTE] If the release exists, it will be upgraded, otherwise it will be installed. If the namespace doesn't exists, it will be created.
...
More configuration examples (i.e. OpenShift) can found seen in [this document](/docs/configuration-examples.md).
...
[Fields#Manage_fields](/docs/manage/fields/#manage-fields) in Sumo Logic UI.
...
[HTTP endpoints](/docs/send-data/hosted-collectors/http-source) in your Sumo Logic account, that are used to send
