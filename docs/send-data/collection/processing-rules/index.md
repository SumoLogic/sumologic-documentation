...
Using processing rules on a [Hosted Collector](/docs/send-data/hosted-collectors) to filter out 50% or more of your data on a single Source may be subject to temporary [throttling](docs/manage/ingestion-and-volume/log-ingestion.md). Excessive filtration can lead to temporary increases in ingest latency. Throttling reduces the effect excessive filtration has on ingestion latency. We encourage you to limit the proportion of data you are filtering out. This policy is subject to change.
