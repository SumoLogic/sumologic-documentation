...
1. Create a [Local File Source](/docs/send-data/installed-collectors/sources/local-file-source). Then, under **File Path**, input the log path specified for `config.log_path`.
   ...
   <img src={useBaseUrl('img/send-data/ruby_on_rails.png')} alt="Ruby on Rails" style={{border: '1px solid gray'}} width="600" />
...
1. Verify that the logs are being ingested. [**New UI**](/docs/get-started/sumo-logic-ui). In the Sumo Logic main menu select **Data Management**, and then under **Data Collection** select **Status**. You can also click the **Go To...** menu at the top of the screen and select **Status**. <br/>[**Classic UI**](/docs/get-started/sumo-logic-ui-classic). In the main Sumo Logic menu, select **Manage Data > Collection > Status**. 
