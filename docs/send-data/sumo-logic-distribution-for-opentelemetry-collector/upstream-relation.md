...
We work closely with the OpenTelemetry community to improve overall experience by:
...
Releasing our own distribution allows us to include Sumo Logic platform-specific features and provide better customer support:
...
In order to prevent confusion we are going to add Sumo Logic specific features when updating the OpenTelemetry Collector version. The OpenTelemetry Collector is released every two weeks and we are releasing up to week after the date.
