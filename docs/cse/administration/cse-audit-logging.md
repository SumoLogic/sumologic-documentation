...
description: Learn how to search the Audit Event Index for SIEM log events.
---

import useBaseUrl from '@docusaurus/useBaseUrl';

The [Audit Event Index](/docs/manage/security/audit-indexes/audit-event-index/) and [System Event Index](/docs/manage/security/audit-indexes/system-event-index/) provide event logs in JSON format on your account activity so you to monitor and audit changes. By default the Audit Event Index and System Event Index are enabled for SIEM and Enterprise accounts.

:::note
This page describes functionality that is available to users whose SIEM URL ends in `sumologic.com`.
:::

## Where to find documentation  

To learn more, see [SIEM audit log definitions](/docs/manage/security/audit-indexes/documentation-audit-log-definitions/#siem-audit-log-definitions).


## Scoping your search

This section explains how to scope a search of the Audit Event Index and System Event Index to return SIEM events.

## Limit search to user or system events

SIEM audit events are stored in two partitions:

* `sumologic_audit_events`. This index contains user action events, which are events that were triggered by a user action, either from the UI or an API. For example, a user created an insight from a signal using the SIEM UI.
* `sumologic_system_events`. This index contains system action events, which are events that were triggered by the system. For example, an insight was generated by SIEM.
...
## Limit search to SIEM events

You can use the `subsystem` field, which every event log contains, to limit the events returned to SIEM-related events:
...
## Limit search by SIEM feature

The table below shows the `_sourceCategory` that is assigned to event logs by SIEM feature.
...
| Assigned insight emails<br/>(Relates to the option on the Actions page that causes a user to receive an email whenever another user assigns an insight to them.) | `cseConfiguration` |
...
| [Insight](/docs/cse/get-started-with-siem/about-cse-insight-ui) | `cseInsight` |
...
## Search for SIEM events 

To search the Audit Event Index or System Event Index for logs that describe SIEM events:
