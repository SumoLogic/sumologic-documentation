...
This topic has information about writing custom SIEM rules.
...
* [Record Processing Pipeline](/docs/cse/schema/record-processing-pipeline). This topic describes how SIEM creates records for incoming messages. It provides facts about how message fields are mapped to SIEM schema attributes; about the attributes SIEM adds to records to enrich and provide context about IP address, URLs, and domains; “list” features, like Match Lists and Suppress Lists that allow you to include or exclude records based on identifiers found in records; how to leverage threat intel data and more.
* [SIEM Schema Attributes](/docs/cse/schema/schema-attributes). This topic defines the record attributes you can reference in rules.
* [SIEM Rules Syntax](/docs/cse/rules/cse-rules-syntax). This topic describes rules language functions and syntax, which you’ll use in writing rule expressions.
* [Searching for SIEM Records in Sumo Logic](/docs/cse/records-signals-entities-insights/search-cse-records-in-sumo). This topic explains how to search SIEM records in the Sumo Logic platform. Typically, you’ll build and refine your rule expressions in Sumo Logic. Once you’re happy with the results, you’ll copy the query into the rule expression field in the rules editor.
...
1. Now we have a query we can use as the basis of an expression for our rule. Note that when you paste it into the rules editor, you should remove the first portion of the query (`_index=sec_record_*` and `| where`), which is only necessary when you are querying records in Sumo Logic. The expression is then as follows:
   ...
   Also ensure that the syntax of the expression matches what is needed by the [SIEM rules syntax](/docs/cse/rules/cse-rules-syntax/). Once you are satisfied that the expression is ready, click **Test Rule Expression** to verify that the expression returns expected results.
