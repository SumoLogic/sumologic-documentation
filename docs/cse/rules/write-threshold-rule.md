...
This topic has information about the threshold rules and how to create them in the SIEM UI.
...
<Iframe url="https://fast.wistia.net/embed/iframe/04ymrxo93q?web_component=true&seo=true&videoFoam=false"
  width="854px"
  height="480px"
  title="Micro Lesson: Create a Threshold Rule in SIEM Video"
  id="wistiaVideo"
...
1. **When a Record matches the expression**. Enter the rule expression, a boolean expression that when “true”, causes the rule to fire.
    :::note
    You can expand the field template guide, which contains a list of all the fields that SIEM can normalize to v3 of the SIEM Schema. Note that the existence of a field in the guide does not mean that your ingested records necessarily include that field.
    :::
...
## Save as prototype
If you are not sure that your rule is ready for use in production, you can save it as a prototype. A prototype rule generates signals, but those signals will not contribute to insights. (Signals generated by a prototype rule do not increment the rule's **On Entity** entity's Activity Score.) Running the rule as a prototype for a while allows you to determine whether the rule is too noisy and fires too many signals.
