...
description: Learn about the contents of the insights UI in SIEM.
---

import useBaseUrl from '@docusaurus/useBaseUrl';
import Iframe from 'react-iframe'; 

This topic describes the SIEM UI for working with insights. 

## Insights list page

[**New UI**](/docs/get-started/sumo-logic-ui). To open the insights page, in the main Sumo Logic menu select **SIEM > Insights**. You can also click **Go To...** at the top of the screen and select **Insights**.
...
:::note
SIEM displays insights and the signals attached to them in the SIEM UI through the end of the data retention period defined in your account agreement. 
:::
...
* **Severity**. The severity of the insight. The value is a function of the configured entity activity score threshold for insight generation. For more information, see [About insight severity](/docs/cse/get-started-with-siem/insight-generation-process#about-insight-severity).
...
You can switch back to the list view by clicking the **Show List** icon, near the top right corner of the SIEM UI.
...
<Iframe url="https://fast.wistia.net/embed/iframe/zqzw9krbcs?web_component=true&seo=true&videoFoam=false"
  width="854px"
  height="480px"
  title="Micro Lesson: SIEM Bulk Insight Management"
  id="wistiaVideo"
...
1. Choose the action you want to take on the selected insights:
   ...
   * **Assign To**. Assign the insights to a SIEM user.
...
Use the following [SIEM APIs](/docs/api/siem-enterprise/) to bulk update insights:
...
If you manage a parent organization with child organizations that also use SIEM, you can see a list of all insights across multiple child organizations. This is useful if your company is a large enterprise with many organizations or is a Managed Security Service Provider (MSSP), and you'd like to see all insights across all areas in a single page.
...
To be able to see insights in child organizations, [add child organizations](/docs/manage/manage-subscription/create-and-manage-orgs/create-manage-orgs) that use SIEM. Then when the parent organization user goes to their SIEM insights list page, all the child organizations' insights appear in the list.


For more information, see [Multi-insights list page in SIEM](/docs/manage/manage-subscription/create-and-manage-orgs/manage-orgs-for-mssps/#multi-insights-list-page-in-siem).

## Insight details page

From the [Heads Up Display](/docs/cse/get-started-with-siem/cse-heads-up-display/) screen, [list view](#list-view), or [board view](#board-view), click an insight's name to see its details.
...
1. **Automations**. Click to view [automations](/docs/cse/automation/automations-in-siem/#view-results-of-an-automation) on the insight.
...
How does SIEM detect entity relationships outside of the insight? Within the time range of the insight, described above, SIEM searches for related [entities in the following normalized record fields](https://github.com/SumoLogic/siem-content-catalog/blob/master/schema/entity_fields.md):
* `*_command`
* `*_deployment`
* `*_domain`
* `*_email`
...
1. **Expand control**. If SIEM has detected additional relationships *outside* of the insight during the selected time frame, an expand/contract control will appear on the entity. Clicking that control will reveal (or hide) those additional relationships. 
...
<Iframe url="https://fast.wistia.net/embed/iframe/7kpacy65bq?web_component=true&seo=true&videoFoam=false"
  width="854px"
  height="480px"
  title="Micro Lesson: SIEM Entity Timeline &amp; Relationship Graph Video"
  id="wistiaVideo"
  className="video-container"
  display="initial"
  position="relative"
...
You can access related entity information using the SIEM API. For more information, see [SIEM APIs](/docs/cse/administration/cse-apis).
