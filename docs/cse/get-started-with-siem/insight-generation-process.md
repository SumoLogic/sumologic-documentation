...
description: Learn how SIEM correlates signals by entity to create insights.
---

import useBaseUrl from '@docusaurus/useBaseUrl';
import Iframe from 'react-iframe'; 

This page explains SIEM's insight generation process. 

The concept of an *entity* is central to the process SIEM uses to correlate signals and create insights. So, what is an entity? In SIEM, an entity is a actor, for example, a  hostname, username, or MAC address encountered in an incoming message. For more information about entities and entity types, see [View and Manage Entities](/docs/cse/records-signals-entities-insights/view-manage-entities).
...
During the next step of the [record processing flow](/docs/cse/schema/record-processing-pipeline)—log mapping—message fields are mapped to SIEM schema attributes. During this process, each entity field from a message is mapped to one of the following [SIEM schema entity attributes](https://github.com/SumoLogic/siem-content-catalog/blob/master/schema/entity_fields.md):
...
SIEM creates an entity when a rule generates a signal, unless the entity already exists in SIEM. When a record matches the conditions of a rule, SIEM generates a separate signal for each On Entity attribute from the record.   
...
## Viewing entities in SIEM UI

You can view the entities that have been extracted from messages on the **Entities** page in the SIEM UI.
...
By default, when an entity’s activity score exceeds the threshold of 12, SIEM generates an insight on the entity. Like the detection period, you can [configure a different activity score threshold value](/docs/cse/records-signals-entities-insights/set-insight-generation-window-threshold) for insight generation. When SIEM creates an insight on an entity, it resets the entity’s activity score to 0.

After SIEM fires a particular signal on a particular entity, it suppresses signals for that signal-entity combination for a time to prevent redundant signals. For more information, see [Redundant signal suppression](#redundant-signal-suppression), below.
...
* You can change the severity of an insight from the severity it was assigned by SIEM at generation time. In the [insight details](/docs/cse/get-started-with-siem/about-cse-insight-ui/) pane, click the icon that appears next to **Severity** to display the severity levels, and select a new level. 

Insights that are generated by the SIEM insight generation algorithm will only have severity levels of Low, Medium, or High. Severity is a function of the entity activity score of the insight’s entity.
