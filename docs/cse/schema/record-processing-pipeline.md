...
description: How SIEM transforms incoming raw messages into records.
---

This topic describes how SIEM transforms incoming raw messages into records. For each message received, SIEM creates a record, or in rare cases, multiple records. 

## Overview of processing steps

A record contains the information from an incoming message, transformed and enhanced in a variety of ways to enable and enhance the SIEM rule evaluation process. The processing steps include:

* Key-value pairs are extracted from messages.
* Source-specific fields are mapped to SIEM schema attributes.
...
For each incoming message, SIEM creates a set of key-value pairs that reflect all of the information in the message. To accomplish this, SIEM provides these ingestion routes:
...
For more information on these alternatives, see [SIEM Ingestion Best Practices](/docs/cse/ingestion/cse-ingestion-best-practices).

The key-value pairs are input to the next step of the process: mapping.

## Map message fields to schema attributes

The [mapping process](/docs/cse/schema/create-structured-log-mapping/) creates a record from the key-value pairs that were extracted from a message, and maps a subset of the keys to SIEM schema attributes. 

Mapping solves a particular problem: messages from different products use different names to identify users, applications, devices and so on. For example, some messages may refer to a source IP address as `sourceIP`, while others use `sourceIpAddress`. We need a standard set of names for the data that most messages are likely to contain. The [SIEM schema](/docs/cse/schema) defines that standard set of names. 

What’s the benefit of mapping? It results in records that use a common (standard) name for fields that hold the same sort of data, regardless of the source of the incoming message. The result: the same SIEM rule can be applied to all records, regardless of the message source.
...
Why normalize? Assume SIEM receives messages with an email-type field `bob@bobsbaitshop.com` and username-type field  `bob`. We can use normalization to transform `bob@bobsbaitshop.com` to `bob`, allowing the username and email to be correlated together.
...
SIEM adds a number of enrichment attributes to records. Enrichment attributes are fields that provide extra context to the record based on attribute values encountered in a record. Specifically, SIEM adds enrichment attributes for IP addresses, URLs, and domains in a record.
...
If a record contains `http_url` or another field that contains an URL, SIEM parses the URL and creates attributes to contain the URL components, like the protocol and top level domain fields. 

For both URLs and domains, SIEM adds a set of enrichment attributes to records, such as entropy calculations for the FQDN and root domain, the Alexa ranking, and so on. 
