...
Parses Windows XML messages from SIEM Windows Sensor. 

## Mapping hints

After parsing, the next step in the SIEM record processing pipeline is log mapping, which is the process of mapping fields that were parsed out of messages to SIEM schema attributes. 

Every parser must provide *mapping hints* that provide information SIEM can use to select the correct log mapper for parsed messages. You do this with the MAPPER attribute. For more information, see [MAPPER](#mapper).
...
Provides information that tells SIEM which log mapper should process the parsed message. There are two ways to do that: 
