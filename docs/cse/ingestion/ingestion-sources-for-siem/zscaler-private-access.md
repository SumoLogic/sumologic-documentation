...
title: Ingest Zscaler Private Access Data into SIEM
sidebar_label: Zscaler Private Access
description: Configure an HTTP source to ingest Zscaler Private Access log messages and send them to SIEM’s Zscaler Private Access system parser.
---

import useBaseUrl from '@docusaurus/useBaseUrl';

To ingest Zscaler Private Access data into SIEM:
1. [Configure a Syslog source](/docs/send-data/installed-collectors/sources/syslog-source/#configure-a-syslog-source) on a collector. When you configure the source, do the following:
    ...
    1. Add another field named `_parser` with value *Parsers/System/Zscaler/Zscaler Private Access/Zscaler Private Access-JSONF*. This ensures that the Zscaler Private Access logs are parsed and normalized into structured records in SIEM.
