---
id: osquery
title: Ingest Osquery Data into SIEM
...
Sumo Logic SIEM supports osquery logs sent in JSON format for the following log types:
...
To ingest osquery data into SIEM:
1. [Configure an HTTP Logs and Metrics source](/docs/send-data/hosted-collectors/http-source/logs-metrics/#configure-an-httplogs-and-metrics-source) on a collector. When you configure the source, do the following:
   ...
   1. Click the **+Add** link to add a field whose name is `_parser` with value */Parsers/System/Osquery/Osquery JSON*. This ensures that the osquery logs are parsed and normalized into structured records in SIEM.
