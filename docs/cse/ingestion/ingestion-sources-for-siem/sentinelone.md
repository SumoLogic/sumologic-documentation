---
id: sentinelone
title: Ingest SentinelOne Data into SIEM
sidebar_label: SentinelOne
description: Learn how to collect SentinelOne log messages and send them to Sumo Logic to be ingested by SIEM.
---

import useBaseUrl from '@docusaurus/useBaseUrl';

This article has instructions for collecting SentinelOne log messages for CEF and Syslog ingest and sending them to Sumo Logic to be ingested by SIEM. (To collect data such as activities, agents, and threats, use the [SentinelOne Mgmt API Source](/docs/send-data/hosted-collectors/cloud-to-cloud-integration-framework/sentinelone-mgmt-api-source/).)

To ingest SentinelOne data into SIEM:
1. [Configure a Syslog source](/docs/send-data/installed-collectors/sources/syslog-source/#configure-a-syslog-source) on a collector. When you configure the source, do the following:
    ...
    1. To ensure that the SentinelOne logs are parsed and normalized into structured records in SIEM, add another field whose name is `_parser` and set the value to:
