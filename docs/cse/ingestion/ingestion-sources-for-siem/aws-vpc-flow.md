---
id: aws-vpc-flow
title: Ingest AWS VPC Flow Data into SIEM
sidebar_label: AWS VPC Flow
description: Configure collection and ingestion of VPC Flow logs from an S3 bucket to be parsed by SIEM's AWS VPC Flow system parser.
---

import useBaseUrl from '@docusaurus/useBaseUrl';

To ingest AWS VPC Flow data into SIEM:
1. [Collect Amazon VPC Flow logs using an Amazon S3 source](/docs/integrations/amazon-aws/vpc-flow-logs/#collecting-amazon-vpc-flow-logs-using-an-amazon-s3-source) on a collector. When you configure the source, do the following:
    ...
    1. Add another field named `_parser` with value */Parsers/System/AWS/AWS VPC Flow*. This ensures that the AWS VPC Flow logs are parsed and normalized into structured records in SIEM.
