---
id: corelight-zeek
title: Ingest Corelight Zeek Data into SIEM
sidebar_label: Corelight Zeek
description: Configure a syslog source to ingest Corelight Zeek log messages and send them to the SIEM Corelight log mapper.
---

import useBaseUrl from '@docusaurus/useBaseUrl';

To ingest Corelight Zeek data into SIEM:
...
1. Configure a Sumo Logic ingest mapping in SIEM for the source category assigned to the source you configured above. The mapping tells SIEM the information it needs to select the right mapper to process messages that have been tagged with that source category. 
    1. [**New UI**](/docs/get-started/sumo-logic-ui). In the main Sumo Logic menu select **SIEM**, and then under **SIEM Integrations** select **Ingest Mappings**. You can also click the **Go To...** menu at the top of the screen and select **Ingest Mappings**.  <br/>[**Classic UI**](/docs/get-started/sumo-logic-ui-classic). In the top SIEM menu select **Configuration**, and then and under **Integrations** select **Sumo Logic**. 
