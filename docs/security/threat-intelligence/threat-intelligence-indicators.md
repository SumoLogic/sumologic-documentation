...
:::note
* The `_sumo_global_feed` source is a default source and cannot be changed or deleted.
* The default storage limit is 10 million total indicators (not including any indicators provided by Sumo Logic such as the `_sumo_global_feed` source).
:::
