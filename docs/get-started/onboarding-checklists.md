...
These topics provide basic information about non-admin Sumo Logic accounts.
...
* [Save Operator](/docs/search/search-query-language/operators#save-lookups-classic): The Save operator allows you to save the results of a query into the Sumo Logic file system. Later, you can use the Lookup operator to access the saved data. The Save operator saves data in a simple format to a location you choose.
* [Lookup Operator](/docs/search/search-query-language/operators#lookup-classic):  Using a lookup operator, you can map data in your log messages to meaningful information. For example, you could use a lookup operator to map "userID" to a real user's name. Or, you could use a lookup operator to find black-listed IP addresses.
* [Join Operator](/docs/search/search-query-language/operators#join): The Join operator combines records of two or more data streams. Results are admitted on-the-fly to allow real time tables to be built. Values common to each table are then delivered as search results. The Join operator in Sumo Logic works much like a standard SQL join.
...
## Getting Started Checklists for Administrators
...
Sumo supports several metric formats: Carbon 2.0, Prometheus, and Graphite. To learn about metrics in Sumo, see [Overview of Metrics in Sumo](/docs/metrics/introduction). Metrics-related features for administrators include:
