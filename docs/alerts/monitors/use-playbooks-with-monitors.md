...
To create a monitor to detect suspicious behavior (anomalies), see [Create an anomaly monitor that runs an automated playbook](#create-an-anomaly-monitor-that-runs-an-automated-playbook) below.
...
You can create a monitor that triggers an alert when unusual behavior is encountered (an anomaly) and runs an automated playbook in response.
