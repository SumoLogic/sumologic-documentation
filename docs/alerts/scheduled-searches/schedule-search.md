...
1. Select a run frequency. When you make a selection, the additional settings are displayed.
    ...
    * **Custom Cron**. Enter a custom CRON expression. The run frequency for a CRON expression must not be less than every 15 minutes. For details, see [Cron Examples and Reference] (../../../03Send-Data/Sources/01Sources-for-Installed-Collectors/Script-Source/Cron-Examples-and-Reference.md "Cron Examples and Reference").  
