...
1. **Run frequency.**<br/><img src={useBaseUrl('img/alerts/run-frequency-1.png')} alt="run frequency" width="670"/>
    ...
    * **Hourly**. The search will run every hour. We guarantee that hourly searches run every hour but not exactly at :00. May be delayed up to 10 minutes past the selected time or interval but will maintain the selected run frequency.
