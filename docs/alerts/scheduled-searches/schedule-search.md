...
You can add up to 10 alert types to a single Scheduled Search, mixing and matching any combination of the types below instead of choosing just one. For example, add an Email alert and a Webhook alert to the same Scheduled Search, or add multiple Email alerts with different recipients. All alert types you add share the trigger condition from [Step 5](#step-5-notification-settings) and fire from the same search execution, but each alert's own configuration, such as email recipients or webhook payload, is set independently.
