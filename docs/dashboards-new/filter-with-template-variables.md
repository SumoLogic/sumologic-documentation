...
When including** **the option to select all values (\*), use the [matches](../search/search-query-language/search-operators/matches) operator. Otherwise `*` will not do an "all" filter, it will filter against the literal asterisk "\*" string.
