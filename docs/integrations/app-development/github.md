...
2. Configure an[ HTTP Source](/docs/send-data/hosted-collectors/http-source/logs-metrics) on the Hosted Collector.
    ...
    * Click **+Add Field** and provide the following:
        ...
        * **Value:** `true`
    * Expand **Advanced Options for Logs (Optional)** section, then ***uncheck (disable)*** option **Extract timestamp information from log file entries** in **Timestamp Parsing** 
