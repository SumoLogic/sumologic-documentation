...
* Filter out unwanted logs before you use the `lookup` operator
* Use keywords
...
For more information, see [Use the lookup search operator](/docs/security/threat-intelligence/find-threats/#use-the-lookup-search-operator).
...
## FAQ for the Threat Intel Quick Analysis app

#### What does the Threat Intel Quick Analysis app do?

This app [uses the lookup search operator](/docs/security/threat-intelligence/find-threats/#use-the-lookup-search-operator) to scan all Sumo Logic logs and parses (using regex) IP/Email/URL/Domain/File Name fields for comparison against the [Sumo Logic Global Feed from CrowdStrike](/docs/security/threat-intelligence/sumologic-global-feed-from-crowdstrike/). Think of it as an inner join between parsed fields and the threat table.

<!-- Per DOCS-643, replace the preceding paragraph with the following after `sumo://threat/cs` is replaced by `threatlookup`:

This app [uses the threatlookup search operator](/docs/security/threat-intelligence/find-threats) to scan all Sumo Logic logs and parses (using regex) IP/Email/URL/Domain/File Name fields for comparison against the `SumoLogic_ThreatIntel` threat intelligence source. Think of it as an inner join between parsed fields and the threat table.
-->
...
Yes. You can scan for malicious Indicators of Compromise (IOCs) in real time. See [Find Threats with Log Queries](/docs/security/threat-intelligence/find-threats/#use-the-lookup-search-operator).
