...
* Enable Elastic Load Balancing logging in your AWS account, using these Sumo Logic [instructions](/docs/send-data/Sources/hosted-collectors/amazon-aws/AWS-Elastic-Load-Balancing-Source). For more information, see [AWS ELB documentation](http://docs.aws.amazon.com/elasticloadbalancing/latest/classic/enable-access-logs.html). Logging is not enabled in AWS ELB by default.
* Grant access to an IAM user by following these Sumo Logic [instructions](/docs/send-data/sources/hosted-collectors/amazon-aws/grant-access-aws-product.md).
...
Configure an [AWS Elastic Load Balancing Source](/docs/send-data/Sources/hosted-collectors/amazon-aws/AWS-Elastic-Load-Balancing-Source), which includes specifying the following:
