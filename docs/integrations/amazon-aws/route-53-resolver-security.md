...
1. Create an [AWS Kinesis Firehose for Logs Source](/docs/send-data/sources/hosted-collectors/amazon-web-services/aws-kinesis-firehose-logs-source). Make a note of the **HTTP Source Address** for the source. You'll need it for the Cloudformation template below.
2. Set up CloudWatch to stream logs to Kinesis Data Firehose using the [Cloudformation Template](/docs/send-data/sources/hosted-collectors/amazon-web-services/aws-kinesis-firehose-logs-source#CloudFormation_Template)
