...
To create a partition, follow the instructions to [Create a Partition](https://help.sumologic.com/Manage/Partitions_and_Data_Tiers/03-Add-a-Partition). Name the index **aws_config**. For the Routing Expression, enter a query that isolates messages from AWS Config, such as `_sourceCategory=aws_config`.
