...
Our new app install flow is now in Beta. It is only enabled for certain customers while we gather Beta customer feedback. If you can see the Add Integration button, follow the "Before you begin" section and then use the in-product instructions in Sumo Logic to set up the app.
...
Now that you have set up collection for CloudFront, install the Sumo Logic App for Amazon CloudFront to use the pre-configured searches and [dashboards](#viewing-dashboards) that provide visibility into your environment for real-time analysis of overall usage.
