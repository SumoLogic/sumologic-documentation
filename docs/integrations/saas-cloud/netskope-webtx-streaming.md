...
title: Netskope WebTx Streaming (Private Preview)
...
| `Netskope WebTx Streaming - Unauthorized Access Attempts` | This alert is triggered when multiple failed login attempts or resource access failures are detected, highlighted by status codes such as “401 Unauthorized” or unusually frequent attempts from a single IP address. | Critical | Count > 2|
