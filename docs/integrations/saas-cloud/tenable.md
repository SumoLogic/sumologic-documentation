...
- **Enhanced Security Posture**. By combining data from the Tenable Vulnerabilities API, Audit Events API and Asset Events API, you can strengthen your organization's security posture by proactively addressing vulnerabilities and monitoring the security incidents.
...
The **Tenable - Asset Inventory** dashboard provide view of Tenable asset data including operating system distribution, network segmentation, scan sources, and geo location. Use this dashboard to track asset coverage and inventory health.
...
| `Tenable - Multiple High or Critical Severity Detections from Single Host` | Monitors and highlights high or critical severity detections for single host. | Critical | Count > 0 |
