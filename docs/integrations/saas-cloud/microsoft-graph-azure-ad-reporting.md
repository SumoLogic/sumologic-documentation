...
|:--|:--|:--|
| `Microsoft Graph Azure AD Reporting - Abnormal Multi-Device Sign-In Activity Detected` | Detects abnormal sign-in patterns where a single user account is used across multiple unique devices within a timeframe. This behavior could be indicative of credential stuffing, session hijacking, or unauthorized access from multiple threat actors. | Count > 5 |
| `Microsoft Graph Azure AD Reporting - Audit Activity Detected from Embargoed Geo Location` | Detects administrative or audit activity initiated from embargoed or restricted regions. Generates alerts to highlight potential policy breaches or suspicious cross-border access. | Count > 0 |
| `Microsoft Graph Azure AD Reporting - High-Risk Sign-In Detected` | Identifies user sign-in events flagged as high risk by Azure AD Identity Protection. Supports investigation of possible credential compromise or unauthorized account use. | Count > 0 |
| `Microsoft Graph Azure AD Reporting - Multiple Consecutive Failed Sign-Ins Detected` | Triggers an alert when repeated failed sign-in attempts occur for a single user or account. Helps identify brute-force attempts, credential guessing, or account lockout risk. | Count > 5 |
| `Microsoft Graph Azure AD Reporting - Repeated Failed Operations on Source System` | Monitor triggers when multiple failed activities are performed on the same source, helping to quickly identify issues and take the necessary corrective actions. | Count > 5 |
| `Microsoft Graph Azure AD Reporting - Repeated Failed Operations on Target Resource` | Monitor triggers when multiple failed activities are performed on the same target. helping to quickly identify issues and take the necessary corrective actions. | Count > 5 |
| `Microsoft Graph Azure AD Reporting - Sign-In Attempt from Embargoed Location` | Tracks Azure AD sign-in activities originating from embargoed or restricted regions. Triggers an alert when any user authentication attempt is detected from these high-risk locations, helping identify potential unauthorized access or policy violations promptly. | Count > 0 |
