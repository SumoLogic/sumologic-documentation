...
By default, the collector will not send process metrics to Sumo Logic; the number of processes running on a host can be very large, which would result in significant increase in Data Points per Minute (DPM).
