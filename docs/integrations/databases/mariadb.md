...
Ssee [this doc](/docs/send-data/collect-from-other-data-sources/collect-metrics-telegraf/install-telegraf#Configuring-Telegraf) for more parameters that can be configured in the Telegraf agent globally.
...
Make sure that the logs from MariaDB are sent to stdout. Follow the instructions below to capture MariaDB logs from stdout on Kubernetes.
...
1. **Add an FER to normalize the fields in Kubernetes environments** Labels created in Kubernetes environments automatically are prefixed with pod_labels. To normalize these for our app to work, we need to create a Field Extraction Rule if not already created for Proxy Application Components:
...
2. Setup Telegraf
   ...
   [[inputs.mysql]]
     ...
     interval_slow = "30m"
   [inputs.mysql.tags]
       environment="dev_TO_BE_CHANGED"
       component="database"
       db_system="mariadb"
       db_cluster="mariadb_on_premise_TO_BE_CHANGED"
   [[outputs.sumologic]]
...
The Terraform script installs the alerts without any scope filters, if you would like to restrict the alerts to specific clusters or environments, update the variable `mariadb_data_source`. Custom filter examples:
   * A specific cluster `db_cluster=mariadb.prod.01`
...
1. **Email and Connection Notification Configuration Examples**. Modify the file **`MariaDB_notifications.auto.tfvars` and populate `connection_notifications` and `email_notifications` as per below examples.
