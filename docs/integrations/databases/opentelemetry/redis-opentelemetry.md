...
The Redis INFO command returns information and statistics about a Redis server (see https://redis.io/commands/info for details). The Redis receiver extracts values from the result and converts them to open telemetry metrics.
...
Use this dashboard to:
- Review keyspace hit, connected clients, Uptime, Evicted vs Expired Keys and Commands Per Seconds.
...
Use this dashboard to:
- Review keyspace hit, Cache miss rate by host, CPU - System vs User usage by host and Network Usage - Inbound/Outbound by Host.
...
Use this dashboard to:
- Review Average Master Uptime, Connected Slaves, Redis Uptime by Host, Commands by Host and Instantaneous OPS per sec by Host.
...
Use this dashboard to:
- Review String Commands, Key Commands, Hash Commands, Sets Commands, Sorted Sets Commands, List Commands, Transaction Commands and Scripting Commands.
