...
2. Follow the instructions for [Configure an Amazon S3 Source in Sumo Logic](/docs/send-data/Sources/hosted-collectors/Amazon-Web-Services/AWS-S3-Source). When setting up an S3 Source, it's important to specify the correct timestamp field. Follow the next steps to do so.
