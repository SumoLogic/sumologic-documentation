...
In this step you export logs to the Pub/Sub topic you created in the previous step. Google Cloud Firewall runs on Google Cloud VPC, so you will collect VPC logs.
...
1. For resource_type, replace `"<resource_variable>"` with `"gce_subnetwork"`.


14


1. In the **Edit Export** window on the right:
