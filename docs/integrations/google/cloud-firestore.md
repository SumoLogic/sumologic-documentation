...
* Collect **GCP Metrics** using the [GCP Metric](/docs/send-data/hosted-collectors/google-source/gcp-metrics-source/) source. Under the **Services** dropdown, select **Cloud Firestore**. For Google Firestore metrics and dimensions, refer to [Google Firestore metrics](https://cloud.google.com/monitoring/api/metrics_gcp#gcp-firestore).
