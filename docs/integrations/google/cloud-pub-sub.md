...
* Collect **GCP Metrics** using the [GCP Metric](/docs/send-data/hosted-collectors/google-source/gcp-metrics-source/) source. Make sure that you select **Pub/Sub** from the **Services** dropdown. For Google PubSub metrics and dimensions, refer to [Google PubSub metrics](https://cloud.google.com/monitoring/api/metrics_gcp#gcp-pubsub).