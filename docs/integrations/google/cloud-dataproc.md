...
* Collect **GCP Metrics** using the [GCP Metric](/docs/send-data/hosted-collectors/google-source/gcp-metrics-source/) source. Make sure that you select **Cloud Dataproc** from the **Services** dropdown. For Google Dataproc metrics and dimensions, refer to [Google Dataproc metrics](https://cloud.google.com/monitoring/api/metrics_gcp#gcp-dataproc).