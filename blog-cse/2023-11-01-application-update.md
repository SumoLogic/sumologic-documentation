...
To improve the usability of the Signals user interface, the way that records are displayed on Signals generated by multi-record (Threshold, Chain, and Aggregation) Rules has changed. Instead of attaching a sample set of records to the Signal and then providing a Queried Record tab to manually search for additional records, all records that were part of the Signal will be displayed in the UI. (As a result, the Queried Records tab has been removed from the UI.)
