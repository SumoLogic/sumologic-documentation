...
sidebar_label: Generate SIEM Signals With a Scheduled Search
description: You can generate a SIEM Signal with a scheduled search.
---
...
When you configure a Scheduled Search to create SIEM Signals, you are prompted to select a [SIEM Record type](../../cse/schema/cse-record-types.md). The fields returned by your search must match an attribute in the Record
type you select. A field whose name does not match a SIEM attribute will not be populated in the Record created from the Schedule Search results. For more about SIEM attribute names, see [Attributes You Can Map to Records](../../cse/schema/attributes-map-to-records.md).
...
1. **Alert Type**. Select SIEM Signal.
1. The popup refreshes.
...
To view Signals that were created from a scheduled search, run a keyword search on “CIP Scheduled Search” on the **Signals** page in the SIEM UI.
