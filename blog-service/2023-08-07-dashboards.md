...
- **Time Range Inheritance on Adding Panels**. When adding a panel to an existing dashboard from the logs search page, you'll now have the flexibility to specify whether the new panel inherits the time selection of the query as configured or if it inherits the dashboard-level time range. This gives you more control over how time ranges are applied to the new panel.

We believe these enhancements will improve your dashboard experience, giving you better control over time ranges and enhancing your ability to analyze data effectively. See our help documentation [here](/docs/dashboards/set-custom-time-ranges) to learn more about setting time ranges within dashboards and panels. 
