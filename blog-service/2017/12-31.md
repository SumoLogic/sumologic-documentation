...
This is an archive of the 2017 Sumo Logic Service Release Notes. To view the full Service Release Notes archive, [click here](/release-notes-service/archive).
...
<img src={useBaseUrl('img/release-notes/service/archive/12-5-17.gif')} alt="Dashboards"/>
...
<img src={useBaseUrl('img/release-notes/service/archive/12-5-17-2.gif')} alt="Live Tail"/>
...
<table>
  ...
  <tr>
   <td>alt+shift+n</td>
   <td>Duplicate only the current query in new tab.</td>
  </tr>
  <tr>
   <td>alt+shift+t</td>
   <td>Duplicate only the current time range in a new tab.</td>
  </tr>
  <tr>
   <td>alt+shift+q</td>
   <td>Duplicate both the current query and time range in new tab.</td>
  </tr>
</table>
...
**AND, OR, and NOT supported in Metrics Queries.** Logical operators are now supported in the metrics query language to let you specify complex Boolean expressions for metrics. For example, you can now say nodes not “forge” by stating: `!node=forge` and you can specify two specific clusters with OR.  
...
* **Switch between your tasks in Sumo Logic with the tab bar**. The top tab bar allows you to keep multiple pages open at the same time and easily navigate between them. The tabs persist across login sessions, and you can switch context without jumping to new browser tabs or windows. This includes having multiple dashboards open in separate tabs. See [Welcome to the Sumo Logic New UI](/docs/get-started/sumo-logic-ui).
* **New log searches, metrics visualizations, and Live Tail sessions**. It's now more convenient start working with logs or metrics. If you click the + icon in the Tabs area, you'll see options to select search, metrics, or Live Tail. See [Welcome to the Sumo Logic New UI](/docs/get-started/sumo-logic-ui).
...
* **Home page**. The new Home page provides quick access to recently opened dashboards and searches.  See [Welcome to the Sumo Logic New UI](/docs/get-started/sumo-logic-ui).
...
**AWS Elastic Load Balancer - Classic.** The AWS Elastic Load Balancer App has been renamed and updated to provide [new panels and dashboards](/docs/integrations/amazon-aws/classic-load-balancer) such as the Failed Dispatch Monitoring Dashboard to help you better investigate your AWS ELB usage.
...
This capability is called quantization. The quantization interval aligns your time series data to common intervals on the time axis (for example every one minute) to optimize the visualization and performance. See [Metrics Quantization](/docs/metrics/introduction/metric-quantization/).
...
**AWS Elastic Load Balancer - Application App.** This App ingests logs stored in an S3 bucket, giving you the visibility to see the overall health of your Application Load Balancer and Target Groups. Use the Sumo Logic App to analyze raw Application Load Balancer data to investigate the availability of applications running behind Application Load Balancers. See [AWS Elastic Load Balancer - Application App](/docs/integrations/amazon-aws/application-load-balancer).
...
<table>
  <tr>
   <td>Account Size - Daily Average</td>
   <td>Old Multiplier</td>
