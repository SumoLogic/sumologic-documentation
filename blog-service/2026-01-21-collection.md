---
title: Token-Based Authentication for Secure HTTP Logs and Metric Source and OTLP/HTTP Source (Collection)
...
We're excited to introduce secure token-based authentication for the HTTP Logs and Metrics Source and the OTLP/HTTP Source. This new capability allows you to authenticate using a unique token in the request header, maintaining the existing HTTPS endpoint behavior while adding token validation per source.
