...
**Delta and Rate operators for Metrics Queries.** Support has been added for delta and rate operators for metrics queries. For details, see the table of operators in [Metrics Queries](/docs/metrics/metric-queries-alerts).
...
* Sumo Logic Multi-account users may have access to organizations that use different [Password Policies](h/docs/manage/security/set-password-policy). With Multi-account, the password policy data from different organizations is centralized.
