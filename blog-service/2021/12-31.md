...
Update - The [AWS S3 Source](/docs/send-data/hosted-collectors/amazon-aws/aws-s3-source) has the ability to collect objects from AWS S3 buckets that do not have versioning enabled and you cannot provide the ListObjectVersions permission, such as Cisco Umbrella. There is now a radio button with the option to have the Source use AWS versioned APIs.
...
New - We're making [Cloud Flex Credits Subscriptions](/docs/manage/manage-subscription/sumo-logic-credits-accounts) available to self-service trial and free accounts with new monthly and annual subscriptions. Customers who upgrade to a self-service paid subscription from a Trial, POV Trial, or Sumo Free subscription will now upgrade directly to Essentials (credit-based), and have the option of either an annual or monthly subscription. For more information, see [Upgrade a Credits Account](/docs/manage/manage-subscription/upgrade-sumo-logic-credits-account).
...
Update - We've implemented some limitations on the number of metrics metadata tags that Sumo Logic will accept, and the length of metadata keys and values. Limiting the volume and length of metrics metadata ensures that noisy sources of metrics do not inundate your account with excessive metadata. For more information, see [Metrics Metadata Limits](/docs/metrics/manage-metric-volume/data-limits-for-metrics). 
