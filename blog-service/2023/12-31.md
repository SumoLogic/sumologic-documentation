...
To access the new charts, simply navigate to the Traces screen and select a subset of comparable traces (such as the same transaction type). Then, click on **Show Chart**. You can use the dropdown menus to choose from the **duration/errors/spans** (per trace) and **timeseries/histogram** options. With these choices, you'll now have a total of six charts to help you better understand the profile of your traces. [Learn more](/docs/apm/traces/view-and-investigate-traces/#trace-duration-breakdown-chart).
