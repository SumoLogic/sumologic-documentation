...
Once indicators are ingested and appear on the **Threat Intelligence** tab, Cloud SIEM analysts can use them to search logs for threats using the `hasThreatMatch` Cloud SIEM rules language function. In addition, all Sumo Logic users can run queries against the indicators in our global feed to uncover threats.
