#!/usr/bin/env python3
"""Restore sql tags in integrations files where main had sql (general pass)"""
import os
import subprocess
import re

base = '/Users/kpohas/sumologic-documentation'
integrations_dir = os.path.join(base, 'docs/integrations')
changed_total = 0
files_changed = 0

for root, dirs, files in os.walk(integrations_dir):
    for fname in sorted(files):
        ...
        for i, (cur, main) in enumerate(zip(current_lines, main_lines)):
            ...
            # If current has ```sumo and main had ```sql, restore sql
            if re.match(r'^\s*```sumo\s*$', cur_stripped) and re.match(r'^\s*```sql\s*$', main_stripped):
